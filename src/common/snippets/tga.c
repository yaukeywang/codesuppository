 ...
 *
 * $Id: //depot/HeroEngine/CLOTH-MAIN/he_plugins/src/common/snippets/tga.c#1 $
...
gliReadTGA(FILE *fp, char *name)
{
  ...
  vertrev = (char) !(tgaHeader.descriptor & TGA_DESC_VERTICAL);
  ...
  if ((format == IL_BGRA_EXT && bpp != 32) ||
      (format == IL_BGR_EXT && bpp != 24) ||
      ((format == IL_LUMINANCE || format == IL_COLOR_INDEX) && bpp != 8)) {
  ...
  genericImage->cmapFormat = IL_BGR_EXT;  // XXX fix me
