...
public:
  ...
  void gather(void)
  {
    ...
    {
      delete []mMeshes;
      mMeshes    = MEMALLOC_NEW(Mesh *)[mMeshCount];
      Mesh **dst = mMeshes;
      MyMeshVector::iterator i;
      for (i=mMyMeshes.begin(); i!=mMyMeshes.end(); ++i)
      {
  ...
  virtual void        importAnimation(const MeshAnimation &animation)
  {
    ...
    a->mTracks = MEMALLOC_NEW(MeshAnimTrack *)[a->mTrackCount];
...
//    assert(0); // not yet implemented
    ...
    const NxU32 *indices)
  {
    ...
    if ( c->mVertexCount )
    {
      c->mVertices = MEMALLOC_NEW(NxF32)[vertex_count*3];
      memcpy(c->mVertices,vertices,sizeof(NxF32)*vertex_count*3);
    }
    c->mTriCount = tri_count;
    if ( c->mTriCount )
    {
      c->mIndices = MEMALLOC_NEW(NxU32)[tri_count*3];
